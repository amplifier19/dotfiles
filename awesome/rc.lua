...
globalkeys = mytable.join(
    ...
    awful.key({ altkey }, "p", function() os.execute("scrot ~/Pictures/screenshots/%b%d_%H:%M:%S.png") end,
    ...
    -- Tag browsing
    awful.key({ altkey, "Control" }, "h",   awful.tag.viewprev,
              {description = "view previous", group = "tag"}),
    awful.key({ altkey, "Control" }, "l",  awful.tag.viewnext,
...
-- Move client to next, prev tag bindings
globalkeys = mytable.join(globalkeys,
      ...
      awful.key({ altkey, "Shift" }, "l",
		        ...
		        local next_tag_index = (current_tag.index % 5) + 1 
      ...
      awful.key({ altkey, "Shift" }, "h",
                function ()
                    if client.focus then
                        local current_tag = client.focus.first_tag
		        local prev_tag_index =  current_tag.index - 1
			if prev_tag_index <= 0 then
			    prev_tag_index = 5 
			end
		        local prev_tag = client.focus.screen.tags[prev_tag_index]
                        if prev_tag then
